...
from users.views import GetUserView, UpdateSimpleDataView, UpdateUserPasswordView, GetOtherUserView, GetBulkUsersView

urlpatterns = [
    #Get
    path('get_user/', GetUserView.as_view(), name="get_user"),
    path('<str:username>/get_user/', GetOtherUserView.as_view(), name="get_other_user"),
    path('get_bulk_users/', GetBulkUsersView.as_view(), name="get_bulk_users"),
